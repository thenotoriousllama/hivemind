...
function resolveDetected(home, projectRoot) {
  ...
  const cursorInstalled = existsSync6(join10(home, ".cursor"));
  ...
  if (cursorInstalled) {
    out.push(join10(home, ".cursor", "skills-cursor"));
    if (projectRoot) {
      out.push(join10(projectRoot, ".cursor", "skills"));
    }
  }
  return out;
}
function detectAgentSkillsRoots(canonicalRoot, home = homedir7(), projectRoot) {
...
function backfillSymlinks(installRoot) {
  ...
  if (entries.length === 0)
    return;
  const detected = detectAgentSkillsRoots(installRoot, homedir8());
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    if (action === "wrote") {
      ...
      const symlinks = opts.install === "global" ? fanOutSymlinks(skillDir, dirName, detectAgentSkillsRoots(root, homedir8(), void 0)) : [];
      try {
        recordPull({
          dirName,
          name,
