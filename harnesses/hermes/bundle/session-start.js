...
function resolveDetected(home, projectRoot) {
  ...
  const cursorInstalled = existsSync9(join15(home, ".cursor"));
  ...
  if (cursorInstalled) {
    out.push(join15(home, ".cursor", "skills-cursor"));
    if (projectRoot) {
      out.push(join15(projectRoot, ".cursor", "skills"));
    }
  }
  return out;
}
...
function backfillSymlinks(installRoot) {
  ...
  if (entries.length === 0)
    return;
  const detected = detectAgentSkillsRoots(installRoot, homedir11());
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    if (action === "wrote") {
      ...
      const symlinks = opts.install === "global" ? fanOutSymlinks(skillDir, dirName, detectAgentSkillsRoots(root, homedir11(), void 0)) : [];
      try {
        recordPull({
          dirName,
          name,
