...
function resolveDetected(home, projectRoot) {
  ...
  const cursorInstalled = existsSync7(join14(home, ".cursor"));
...
function detectAgentSkillsRoots(canonicalRoot, home = homedir8(), projectRoot) {
...
function backfillSymlinks(installRoot) {
  ...
  if (entries.length === 0)
    return;
  const detected = detectAgentSkillsRoots(installRoot, homedir9());
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    if (action === "wrote") {
      ...
      const symlinks = opts.install === "global" ? fanOutSymlinks(skillDir, dirName, detectAgentSkillsRoots(root, homedir9(), void 0)) : [];
      try {
        recordPull({
          dirName,
          name,
