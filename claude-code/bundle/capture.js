...
// dist/src/hooks/auto-extract-patterns.js
...
// dist/src/hooks/auto-extract.js
async function tryAutoExtract(query, taskEventsTable, input, options) {
  ...
  if (typeof command !== "string")
    return null;
  if (!isToolResponseSuccess(input.tool_response))
    return null;
...
function isToolResponseSuccess(resp) {
  if (!resp)
    return true;
  if (resp.interrupted === true)
    return false;
  if (resp.is_error === true || resp.error === true)
    return false;
  ...
  try {
    ...
    if (msg.includes("does not exist") || msg.includes("permission denied")) {
      try {
        ...
        if (kind)
          log5(`auto-extract: ${kind} (after ensure)`);
      } catch (retryErr) {
        log5(`auto-extract retry failed: ${retryErr?.message ?? String(retryErr)}`);
      }
    } else {
      log5(`auto-extract failed: ${msg}`);
    }
  }
