...
The pull writes canonically to `~/.claude/skills/<name>--<author>/SKILL.md` and fans out symlinks into every detected non-Claude agent skill root (`~/.agents/skills/`, `~/.hermes/skills/`, `~/.pi/agent/skills/`, `~/.cursor/skills-cursor/`, and `<project>/.cursor/skills/` when Cursor is installed) so Codex / Hermes / pi / Cursor discover the same skill without an extra copy on disk. Symlink targets are recorded per-entry in the manifest, so `unpull` reverses the fan-out without rescanning the filesystem. The **Hivemind for Cursor** extension (`harnesses/cursor/extension/`) re-syncs Cursor skill roots on workspace open when developing from a monorepo checkout.
